  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-sm">
        ...
        {/* Mounted only while the dialog is open, so its form state resets on
            every open without an effect watching the `open` prop. */}
        {open ? <WorkspacesForm onClose={onClose} /> : null}
      </DialogContent>
    </Dialog>
  );
}

function WorkspacesForm({ onClose }: { onClose: () => void }) {
